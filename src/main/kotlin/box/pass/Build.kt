...
import box.pass.backend.Stage
import box.pass.frontend.Read
  ...
  suspend operator fun invoke(): Result {
    ...
    return coroutineScope {
      ...
      val elaboratedDefinitions = src
        .walk()
        .filter { it.extension == "box" }
        .mapTo(mutableListOf()) { path ->
          async {
            val location = ModuleLocation(listOf(context.config.name) + src.relativize(path).invariantSeparatorsPathString.dropLast(".box".length).split('/'))
      ...
      val datapacks = (box / config.properties.levelName / "datapacks").createDirectories()
